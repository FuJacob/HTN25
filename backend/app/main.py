...
def create_app() -> FastAPI:
    ...
    async def upload_and_analyze(
        video1: UploadFile = File(...), video2: UploadFile = File(...)
    ):
        ...
        try:
            ...
            prompt = (
                "The first video is the reference. The second video is the user's recording. Analyze the dance movements in the reference video and compare them to the user's recording. Get significant dance moments/moves in the user's recording where it was different from the reference. "
                "For each significant dance moment/move, return a JSON object with: "
                "- timestamp (MM:SS.F format, e.g., '0:05.0'), "
                "- feedback (strictly no more than 2 sentences), "
                "- score (an integer between -10 and 10), "
                "- problem_type (if feedback is negative choose timing, technique, execution, otherwise None). "
            ...
            if analysis_data and hasattr(analysis_data, "dance_analysis"):
                print(
                    f"Successfully parsed {len(analysis_data.dance_analysis)} dance analysis items"
                )
    ...
    async def process_dance_video(
        dance_analysis: str = Form(...), video_file: UploadFile = File(...)
    ):
        print("Received video file:", video_file.filename)
        print(
            "Received dance analysis:",
            dance_analysis[:200] + "..."
            if len(dance_analysis) > 200
            else dance_analysis,
        )
        ...
        try:
            ...
            if (
                "parsed_data" in parsed_json
                and "dance_analysis" in parsed_json["parsed_data"]
            ):
            ...
            try:
                ...
                for i, pattern in enumerate(json_patterns):
                    ...
                    if matches:
                        ...
                        print(
                            f"Found JSON with pattern {i + 1}: {extracted_json[:100]}..."
                        )
                        break
                ...
                if extracted_json:
                    ...
                    if timestamp_matches:
                        ...
                        analysis_data = {"performance_comparison": []}
                        ...
                        print(
                            f"Created structure from timestamps: {len(analysis_data['performance_comparison'])} moves"
                        )
                    else:
        ...
        if analysis_data:
            ...
            if analysis_data.get("dance_analysis"):
                print(
                    f"  - First dance_analysis item: {analysis_data['dance_analysis'][0]}"
                )
            elif analysis_data.get("performance_comparison"):
                print(
                    f"  - First performance_comparison item: {analysis_data['performance_comparison'][0]}"
                )
        else:
        ...
        try:
            ...
            if not process_cap.isOpened() or not display_cap.isOpened():
                raise Exception(
                    "Could not open video file. Please check the video format."
                )
            ...
            if process_fps <= 0 or process_width <= 0 or process_height <= 0:
                raise Exception(
                    f"Invalid video properties: fps={process_fps}, width={process_width}, height={process_height}"
                )

            print(
                f"Process video properties: {process_width}x{process_height} @ {process_fps}fps"
            )
            print(
                f"Display video properties: {display_width}x{display_height} @ {display_fps}fps"
            )
            ...
            print(
                f"Dynamic font sizing: base_scale={base_scale:.3f}, final_scale={calculated_font_scale:.3f}"
            )

            # DEBUG: Check if FPS is reasonable, fix if not
            if process_fps > 100 or process_fps <= 0:
                print(
                    f"WARNING: Detected unusual FPS {process_fps}, correcting to 30fps"
                )
                process_fps = 30
                display_fps = 30
            ...
            print(
                f"DEBUG: Input file size: {os.path.getsize(temp_input_video_path)} bytes"
            )
            ...
            # Group by timestamp and use Gemini's provided score for each moment
            timestamp_groups = {}
            for item in analysis_items:
                timestamp = item.get(
                    "timestamp_of_outcome", item.get("timestamp", "0:00.0")
                )
                ...
                score = item.get("score", 0)  # Use Gemini's score if present
                if result_type == "recording":
                    timestamp_groups[timestamp]["recording_feedback"] = feedback
                    timestamp_groups[timestamp]["score"] = score

            # Convert to dance_events format and calculate final score
            final_score = 0
                ...
                dance_events.append(
                    {
                        ...
                        "feedback_end_frame": timestamp_to_frame(timestamp, process_fps)
                        + (4 * process_fps),
                    }
                )
                final_score += group["score"]

            print(f"Created {len(dance_events)} dance events for annotation")
            print(f"Final score for this video: {final_score}")
            ...
            while process_cap.isOpened() and display_cap.isOpened():
                ...
                if last_move_time is not None:
                    ...
                    if elapsed_time < 1.25:
                        current_color = get_animation_color(
                            elapsed_time, last_move_result
                        )
                    else:
                ...
                if current_feedback and current_moment:
                    ...
                    base_scale = (
                        min(display_width, display_height) / 1200
                    )  # Scale based on smaller dimension
                    feedback_scale = max(
                        0.3, min(0.8, base_scale)
                    )  # Clamp between 0.3 and 0.8
                    feedback_color = (255, 255, 255)
                    feedback_border = (0, 0, 0)
                    feedback_thickness = max(
                        1, int(feedback_scale * 3)
                    )  # Scale thickness too
                    feedback_border_thickness = feedback_thickness + 2
                    feedback_spacing = max(
                        20, int(feedback_scale * 40)
                    )  # Scale spacing

                    # Add moment header to feedback
                    moment_header = f"Moment {current_moment_number}"
                    ...
                    min_start_y = max(
                        50, int(display_height * 0.1)
                    )  # At least 10% from top
                    ideal_start_y = display_height - total_height - 30
                    start_y = max(min_start_y, ideal_start_y)

                    for i, line in enumerate(wrapped_lines):
                        text_size = cv2.getTextSize(
                            line, feedback_font, feedback_scale, feedback_thickness
                        )[0]
            ...
            if test_cap.isOpened():
                ...
                print(
                    f"DEBUG: Output video validation - {test_width}x{test_height} @ {test_fps}fps, {test_frame_count} frames"
                )
    ...
    async def test_structured_output():
        ...
        try:
            ...
            return {
                ...
                "parsed_count": len(analysis_data.dance_analysis)
                if analysis_data
                else 0,
                "sample_data": {
                    "dance_analysis": [
